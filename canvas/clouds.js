function CloudManager(container, options) {
    ...
    var defaults = {
        ...
        radius: 30,
        jitter: 0.5,
        xRange: [width * 1.9, width * 0.1],
        yRange: [height - (height / 4), height - (height / 4)],
        color: {
            r: 255,
            g: 255,
            b: 255
    ...
    for (var i = 0; i < options.count; i++) {
        ...
        icontext.closePath();
        cloud.img = icanvas;
        cloud.speed = Math.random() * 1.15 + 1.049;
        Clouds.push(cloud);
    }
    ...
    function update() {
        for (var i = 0, l = Clouds.length; i < l; i++) {
            ...
            if (Clouds[i].x < Clouds[i].radius * -4) {
                Clouds[i].x = width + Math.random()*256+256; ;
            }
...
setTimeout(function () {
    CloudManager(document.body, {
        jitter: 0.88
    } /* options - see ln 37 for defaults */ );
}, 100)